...
public class EventSetCapzoneArgument extends CommandArgument {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (capturableFaction instanceof ConquestFaction) {
            if (args.length < 3) {
                sender.sendMessage(ChatColor.RED + "Usage: /" + label + ' ' + getName() + ' ' + faction.getName() + " <red|blue|green|WHITE>");
                return true;
            }
        ...
        sender.sendMessage(ChatColor.WHITE + "Set capture zone " + captureZone.getDisplayName() + ChatColor.WHITE + " for faction " + faction.getName() + ChatColor.WHITE + '.');
        return true;
    }
