        ...
        public Prompt acceptInput(final ConversationContext context, final String string) {
            if (string.equalsIgnoreCase("yes")) {
                ...
                if (conversable instanceof CommandSender) {
                    Command.broadcastCommandMessage((CommandSender) conversable, ChatColor.GREEN + "Set EOTW mode to " + newStatus + '.');
                } else conversable.sendRawMessage(ChatColor.GREEN + "Set EOTW mode to " + newStatus + '.');

                HCF.getPlugin().getEotwHandler().setEndOfTheWorld(newStatus);
            } else if (string.equalsIgnoreCase("no")) {
                context.getForWhom().sendRawMessage(ChatColor.RED + "Cancelled the process of setting EOTW mode.");
            } else {
