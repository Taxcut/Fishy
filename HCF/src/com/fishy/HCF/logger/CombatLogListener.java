...
public class CombatLogListener implements Listener {
            ...
            new BukkitRunnable() {
                public void run() {
                    if (combatLogEntry.getKillMessage() != null) {
                        player.sendMessage(ChatColor.RED + "You were killed whilst logged out: ");
    ...
    public void onPlayerQuit(PlayerQuitEvent event) {
        ...
        if ((player.getGameMode() != GameMode.CREATIVE) && (!player.isDead()) && (!SAFE_DISCONNECTS.contains(uuid))) {
            if (!HCF.getPlugin().getConfig().getBoolean("KITMAP") && this.plugin.getTimerManager().getInvincibilityTimer().getRemaining(uuid) > 0L) {
                return;
            }
            ...
            boolean pvpTimer = plugin.getTimerManager().combatTimer.getRemaining(uuid) > 0;
            ...
            LOGGERS.put(uuid, new CombatLogEntry(player, loggerEntity, new LoggerRemovable(uuid, loggerEntity).runTaskLater(this.plugin, (HCF.getPlugin().getConfig().getBoolean("KITMAP") ? 20 * 5 : 20 * 20) + (pvpTimer ? 20 * 15 : 0))));
