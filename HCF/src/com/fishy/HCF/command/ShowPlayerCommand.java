...
public class ShowPlayerCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
		...
		if (args.length == 0 || args.length > 1) {
			sender.sendMessage(ChatColor.RED + "Usage: /showplayer <player>");
			return true;
		}
		...
		if (HCF.getPlugin().getStaffModeListener().isVanished(target)) {
			sender.sendMessage(ChatColor.RED + "You cannot show this player because they are vanished.");
		} else {
			for (Player all : Bukkit.getOnlinePlayers()) {
				all.showPlayer(target);
			}
			sender.sendMessage(ChatColor.GREEN + "You have shown " + target.getName() + '.');
		}
        return true;
    }
