...
public enum ChatChannel {
    ...
    public String getDisplayName() {
        ...
        switch (this) {
            case FACTION:
                prefix = ChatColor.valueOf(plugin.getConfig().getString("TEAMMATE_COLOR")).toString();
                break;
            case ALLIANCE:
                prefix = ChatColor.valueOf(plugin.getConfig().getString("ALLY_COLOR")).toString();
                break;
            case PUBLIC:
            default:
                prefix = ChatColor.valueOf(plugin.getConfig().getString("ENEMY_COLOR")).toString();
                break;
        }
    ...
    public ChatChannel getRotation() {
        switch (this) {
            case FACTION:
                return PUBLIC;
            case PUBLIC:
                return HCF.getPlugin().getConfig().getInt("FACTION.MAXALLIES") > 0 ? ALLIANCE : FACTION;
            case ALLIANCE:
                return FACTION;
            default:
                return PUBLIC;
    ...
    public String getRawFormat(Player player) {
        ...
        switch (this) {
            case FACTION:
                colour = ChatColor.valueOf(plugin.getConfig().getString("TEAMMATE_COLOR"));
                return colour + "(" + getDisplayName() + colour + ") " + player.getName() + ChatColor.GRAY + ": " + ChatColor.WHITE + "%2$s";
            case ALLIANCE:
                colour = ChatColor.valueOf(plugin.getConfig().getString("ALLY_COLOR"));
                return colour + "(" + getDisplayName() + colour + ") " + player.getName() + ChatColor.GRAY + ": " + ChatColor.WHITE + "%2$s";
            default:
