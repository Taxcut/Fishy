package com.fishy.hcf.faction.claim;
...
import com.fishy.hcf.faction.type.WildernessFaction;
import com.fishy.hcf.util.base.ItemBuilder;
import com.fishy.hcf.util.base.cuboid.Cuboid;
import com.fishy.hcf.util.base.cuboid.CuboidDirection;
...
public class ClaimHandler {
    ...
    public static final ItemStack SUBCLAIM_WAND = new ItemBuilder(Material.GOLD_SPADE, 1).displayName(ChatColor.GREEN + "Subclaim Wand").lore(
            ChatColor.GREEN + "Left or Right Click " + ChatColor.WHITE + "a Block" + ChatColor.WHITE + " to:",
            ChatColor.GRAY + "Set the first and second position of ",
            ChatColor.GRAY + "your Subclaim selection.",
            "",
            ChatColor.GREEN + "Right Click " + ChatColor.WHITE + "the Air" + ChatColor.WHITE + " to:",
            ChatColor.GRAY + "Clear your current Subclaim selection.",
            "",
            ChatColor.WHITE + "Use " + ChatColor.GREEN + "/faction subclaim create <name>" + ChatColor.WHITE + " to:",
            ChatColor.GRAY + "Acquire your selected Subclaim.").build();

    public static final ItemStack CLAIM_WAND = new ItemBuilder(Material.DIAMOND_HOE).displayName(ChatColor.GREEN + "Claim Wand").lore(
            ChatColor.GREEN + "Left or Right Click " + ChatColor.WHITE + "a Block" + ChatColor.WHITE + " to:",
            ChatColor.GRAY + "Set the first and second position of ",
            ...
            "",
            ChatColor.GREEN + "Shift " + ChatColor.GREEN + "Left Click " + ChatColor.WHITE + "the Air or a Block" + ChatColor.WHITE + " to:",
    ...
    public boolean tryCreatingSubclaim(Player player, Subclaim subclaim) {
        ...
        player.sendMessage(ChatColor.GREEN + "You have created a subclaim named " + ChatColor.GRAY + subclaim.getName() + ChatColor.GREEN + '.');
        return true;
    }
    ...
    public boolean canClaimHere(Player player, Location location) {
        ...
        if (!(plugin.getFactionManager().getFactionAt(location) instanceof WildernessFaction)) {
            player.sendMessage(ChatColor.RED + "You can only claim land in the " + ChatColor.valueOf(plugin.getConfig().getString("WILDERNESS_COLOR")) + "Wilderness" + ChatColor.RED + ". " +
                    "Make sure you are past " + plugin.getConfig().getInt("WARZONERADIUS") + " blocks " + ChatColor.RED + "from spawn...");
        ...
        if (playerFaction.getClaims().size() >= plugin.getConfig().getInt("FACTION.MAXCLAIMS")) {
            player.sendMessage(ChatColor.RED + "Your faction has maximum claims possible, which is " + plugin.getConfig().getInt("FACTION.CLAIMS") + ".");
            return false;
        }
    ...
    public boolean tryPurchasing(Player player, Claim claim) {
        ...
        if (playerFaction.getClaims().size() >= plugin.getConfig().getInt("FACTION.MAXCLAIMS")) {
            player.sendMessage(ChatColor.RED + "Your faction has maximum claims possible, which is " + plugin.getConfig().getInt("FACTION.MAXCLAIMS") + ".");
            return false;
        }
        ...
        player.sendMessage(ChatColor.WHITE + "Claim has been purchased for " + ChatColor.GREEN + EconomyManager.ECONOMY_SYMBOL + claimPrice + ChatColor.WHITE + '.');
        playerFaction.setBalance(factionBalance - claimPrice);
        playerFaction.broadcast(ChatColor.GREEN + player.getName() + ChatColor.WHITE + " claimed land for your faction at " +
                ChatColor.GRAY + '(' + center.getBlockX() + ", " + center.getBlockZ() + ')' + ChatColor.WHITE + '.', player.getUniqueId());
