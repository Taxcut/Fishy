...
import com.fishy.hcf.util.base.MapSorting;
...
public class FactionListArgument extends CommandArgument {
    ...
    private void showList(final int pageNumber, final String label, final CommandSender sender) {
        ...
        for (Player target : Bukkit.getOnlinePlayers()) {
            if (senderPlayer == null || senderPlayer.canSee(target) || !senderPlayer.canSee(target)) {
        ...
        for (Map.Entry<PlayerFaction, Integer> entry : sortedMap) {
            ...
            builder.append(displayName).color(net.md_5.bungee.api.ChatColor.RED).
                    ...
                    event(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder(net.md_5.bungee.api.ChatColor.WHITE + "Click to view " + displayName + ChatColor.WHITE + '.')
                            .create()));
            ...
            builder.append(" " + JavaUtils.format(playerFaction.getDeathsUntilRaidable())).color(net.md_5.bungee.api.ChatColor.WHITE);
            builder.append('/' + JavaUtils.format(playerFaction.getMaximumDeathsUntilRaidable()) + " DTR").color(net.md_5.bungee.api.ChatColor.WHITE);
        ...
        sender.sendMessage(ChatColor.GREEN.toString() + ChatColor.BOLD + "Faction List " + ChatColor.GRAY + "(Page " + pageNumber + '/' + maxPages + ')');
