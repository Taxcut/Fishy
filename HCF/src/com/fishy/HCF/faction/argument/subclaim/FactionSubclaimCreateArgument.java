...
import com.fishy.hcf.faction.claim.ClaimSelection;
...
public class FactionSubclaimCreateArgument extends CommandArgument {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args[2].length() < plugin.getConfig().getInt("FACTION.MINIMUMNAMECHARACTERS")) {
            sender.sendMessage(ChatColor.RED + "Subclaim names must have at least " + plugin.getConfig().getInt("FACTION.MINIMUMNAMECHARACTERS") + " characters.");
            return true;
        }

        if (args[2].length() > plugin.getConfig().getInt("FACTION.MAXNAMECHARACTERS")) {
            sender.sendMessage(ChatColor.RED + "Subclaim names cannot be longer than " + plugin.getConfig().getInt("FACTION.MAXNAMECHARACTERS") + " characters.");
            return true;
        }
