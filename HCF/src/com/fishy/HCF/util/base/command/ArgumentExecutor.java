package com.fishy.hcf.util.base.command;
...
public abstract class ArgumentExecutor implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length < 1) {
            ...
            sender.sendMessage(ChatColor.DARK_AQUA.toString() + ChatColor.BOLD + WordUtils.capitalizeFully(this.label) + " Help");
            for (CommandArgument argument : arguments) {
                String permission = argument.getPermission();
                if (permission == null || sender.hasPermission(permission)) {
                    sender.sendMessage(ChatColor.AQUA + argument.getUsage(label) + ChatColor.GRAY + " - " + ChatColor.WHITE + argument.getDescription() + '.');
                }
            }
        	sender.sendMessage(ChatColor.DARK_GRAY + BukkitUtils.STRAIGHT_LINE_DEFAULT);
