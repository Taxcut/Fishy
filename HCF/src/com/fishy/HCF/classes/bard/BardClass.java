...
public class BardClass extends PvpClass implements Listener {
    ...
    public BardClass(HCF plugin) {
        ...
        this.bardEffects.put(Material.GOLD_INGOT, new BardEffect(45,
                new PotionEffect(PotionEffectType.INVISIBILITY, 600, 0),
                new PotionEffect(PotionEffectType.INVISIBILITY, HELD_EFFECT_DURATION_TICKS, 0)));
        this.bardEffects.put(Material.BLAZE_POWDER, new BardEffect(50,
                new PotionEffect(PotionEffectType.INCREASE_DAMAGE, 120, 1), // change the amplifier here for clickable
        ...
        this.bardEffects.put(Material.FEATHER, new BardEffect(40,
                new PotionEffect(PotionEffectType.JUMP, 120, 4),
                new PotionEffect(PotionEffectType.JUMP, HELD_EFFECT_DURATION_TICKS, 1)));
        this.bardEffects.put(Material.SPIDER_EYE, new BardEffect(35,
    ...
    public boolean onEquip(Player player) {
        if (this.plugin.getTimerManager().getInvincibilityTimer().getRemaining(player) > 0L) {
            player.sendMessage(ChatColor.RED + "You cannot equip bard class whilst your " + ChatColor.GREEN + ChatColor.BOLD.toString() + "PVP Timer" + ChatColor.RED + " is active.");
            return false;
        }
            ...
            public void run() {
                ...
                if (held != null) {
                    ...
                    if (bardEffect.heldable.getType() == PotionEffectType.JUMP) {
                        plugin.getEffectRestorer().setRestoreEffect(player, new PotionEffect(PotionEffectType.JUMP, 120, 1));
                    }
                    
                    if (bardEffect.heldable.getType() == PotionEffectType.INVISIBILITY) {
                        plugin.getEffectRestorer().setRestoreEffect(player, new PotionEffect(PotionEffectType.INVISIBILITY, 120, 1));
                    }
                ...
                if (energy != 0 && energy != lastEnergy && (energy % 10 == 0 || lastEnergy - energy - 1 > 0 || energy == BardData.MAX_ENERGY)) {
                    lastEnergy = energy;
                    //player.sendMessage(ChatColor.YELLOW + name + " energy is now at " + ChatColor.RED + energy + ChatColor.GOLD + '.');
                }
            }
...
/*                player.sendMessage(ChatColor.DARK_GRAY + BukkitUtils.STRAIGHT_LINE_DEFAULT);
                player.sendMessage(ChatColor.GRAY + " � " + ChatColor.WHITE + "Clickable Effect: " + ChatColor.AQUA +
                ...
                player.sendMessage(ChatColor.GRAY + " � " + ChatColor.WHITE + "Energy Cost: " + ChatColor.AQUA + bardEffect.energyCost);
                player.sendMessage(ChatColor.DARK_GRAY + BukkitUtils.STRAIGHT_LINE_DEFAULT);*/
            }
        }
    }
        ...
        if (action == Action.RIGHT_CLICK_AIR || (!event.isCancelled() && action == Action.RIGHT_CLICK_BLOCK)) {
            ...
            if (bardData != null) {
                ...
                if ((bardEffect != null) && (!this.plugin.getFactionManager().getFactionAt(player.getLocation()).isSafezone())) {
                    ...
                    else if (bardEffect.clickable.getType().equals(PotionEffectType.INVISIBILITY)) {
                ...
                player.sendMessage(ChatColor.WHITE + "You have just used " + name + " buff " + ChatColor.GREEN +
                        Lang.fromPotionEffectType(bardEffect.clickable.getType()) + ' ' + (bardEffect.clickable.getAmplifier() + 1) + ChatColor.WHITE + " costing you " +
                        ChatColor.BOLD + bardEffect.energyCost + ChatColor.WHITE + " energy. " +
                        "Your energy is now " + ChatColor.GREEN + ((newEnergy * 10.0) / 10.0)/*TODO:neeeded?*/ + ChatColor.WHITE + '.');
                if (plugin.getSotwTimer().getSotwRunnable() == null) {
                    this.plugin.getTimerManager().getCombatTimer().setCooldown(player, player.getUniqueId());
                }
            }
        }
    }
    ...
    private boolean canUseBardEffect(Player player, BardData bardData, BardEffect bardEffect, boolean sendFeedback) {
        ...
        if (remaining > 0L) {
            errorFeedback = ChatColor.RED + "You cannot use this " + ChatColor.GREEN.toString() + ChatColor.BOLD + "Bard " + ChatColor.RED + "buff for another " + DurationFormatter.getRemaining(remaining, true, false) + ChatColor.RED + '.';
        }
