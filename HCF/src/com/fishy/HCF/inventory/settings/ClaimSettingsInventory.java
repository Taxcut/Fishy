package com.fishy.hcf.inventory.settings;
...
import com.fishy.hcf.inventory.CustomInventory;
import com.fishy.hcf.inventory.CustomInventoryListener;
import com.fishy.hcf.inventory.settings.runnable.BlockChangeRunnable;
import com.fishy.hcf.inventory.settings.runnable.BlockChangeRunnable.BlockOperation;
import com.fishy.hcf.util.ItemUtil;
import com.fishy.hcf.util.base.ItemBuilder;

import net.md_5.bungee.api.ChatColor;

public class ClaimSettingsInventory extends CustomInventory<HCF> {

	private static final ItemStack COBBLE_BOX = new ItemBuilder(Material.COBBLE_WALL)
			.displayName(ChatColor.GREEN + "Generate Cobblestone Box").build();
	private static final ItemStack STONE_BOX = new ItemBuilder(Material.STONE)
			.displayName(ChatColor.GREEN + "Generate Stone Box").build();
	private static final ItemStack DIRT_BOX = new ItemBuilder(Material.DIRT)
			.displayName(ChatColor.GREEN + "Generate Dirt Box").build();
	private static final ItemStack NETHERACK = new ItemBuilder(Material.NETHERRACK)
			.displayName(ChatColor.GREEN + "Generate Netherack Box").build();

	public ClaimSettingsInventory(HCF plugin) {
		super(plugin, ChatColor.GREEN + "Claim Settings", 27);
	}
		...
		this.listeners.add(new CustomInventoryListener() {
			...
			public void onClick(InventoryClickEvent event) {
				...
				if (item.getType() == Material.COBBLE_WALL) {
					...
					Set<Claim> claims = (Set<Claim>) faction.getClaims();
					...
					new BlockChangeRunnable(BlockOperation.X, Material.DIRT, minY, min, max, 1)
							.runTaskTimer(plugin, 0, 1);
					new BlockChangeRunnable(BlockOperation.Z, Material.DIRT, minY, min, max, 1)
							.runTaskTimer(plugin, 0, 1);
